...
@section('title', 'Tambah Produk Rokok')
@section('header_title', 'Form Tambah Produk Rokok Baru')
    ...
    <form action="{{ route('admin.barangs.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            <div>
                ...
                <input type="text" name="nama" value="{{ old('nama') }}" required placeholder="Contoh: Kencana Merah 12"
            ...
            <div>
                ...
                <input type="text" name="kode_barang" value="{{ old('kode_barang') }}" required placeholder="Contoh: SKM-KM12"
                       style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px;">
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            ...
            <div>
                ...
                <select name="status_stok" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px;">
                    <option value="Ready Stock" {{ old('status_stok') == 'Ready Stock' ? 'selected' : '' }}>Ready Stock</option>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" name="profil_rasa" value="{{ old('profil_rasa') }}" placeholder="Contoh: Full flavour, manis cengkeh pekat, after taste hangat..."
                   style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px;">
        </div>

        <div style="margin-bottom: 20px;">
            <label style="display: block; font-weight: 600; font-size: 13px; margin-bottom: 6px;">Deskripsi Narasi Produk</label>
            <textarea name="deskripsi" rows="3" placeholder="Penjelasan mengenai bahan baku tembakau, racikan saus cengkeh, dll..."
        ...
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 16px; margin-bottom: 20px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 16px;">
                <div>
                    ...
                    <input type="number" name="batang_per_bungkus" value="{{ old('batang_per_bungkus', 12) }}" required min="1"
                ...
                <div>
                    ...
                    <input type="number" name="bungkus_per_slop" value="{{ old('bungkus_per_slop', 10) }}" required min="1"
                           style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px;">
                </div>
                <div>
                    <label style="font-size: 12px; font-weight: 600; display: block; margin-bottom: 4px;">Slop / Bal</label>
                    <input type="number" name="slop_per_bal" value="{{ old('slop_per_bal', 20) }}" required min="1"
                           style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px;">
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 16px;">
                <div>
                    ...
                    <input type="number" name="harga_per_bal" id="inputHargaBal" value="{{ old('harga_per_bal', 1000000) }}" required min="0"
                           style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px;">
                </div>
                <div>
                    <label style="font-size: 12px; font-weight: 600; display: block; margin-bottom: 4px;">Harga Slop (Rp)</label>
                    <input type="number" name="harga_per_slop" id="inputHargaSlop" value="{{ old('harga_per_slop', 50000) }}" min="0" placeholder="Otomatis"
                           style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px;">
                ...
                <div>
                    ...
                    <input type="number" name="min_order_slop" value="{{ old('min_order_slop', 1) }}" required min="1"
                           style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px;">
                </div>
                <div>
                    <label style="font-size: 12px; font-weight: 600; display: block; margin-bottom: 4px;">Min. Order Bal</label>
                    <input type="number" name="min_order_bal" value="{{ old('min_order_bal', 1) }}" required min="1"
                           style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px;">
                </div>
            </div>

            <div>
                <label style="font-size: 12px; font-weight: 600; display: block; margin-bottom: 4px;">Stok Awal (Bal)</label>
                <input type="number" name="stok" value="{{ old('stok', 100) }}" required min="0"
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            <div>
                ...
                <input type="number" step="0.1" name="tar_mg" value="{{ old('tar_mg', 28.0) }}"
                       style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 6px;">
            </div>
            <div>
                <label style="display: block; font-weight: 600; font-size: 13px; margin-bottom: 6px;">Kadar Nikotin (mg)</label>
                <input type="number" step="0.1" name="nikotin_mg" value="{{ old('nikotin_mg', 1.8) }}"
                       style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 6px;">
            </div>
        </div>

        <div style="margin-bottom: 24px;">
            <label style="display: block; font-weight: 600; font-size: 13px; margin-bottom: 6px;">Foto Produk (Upload Gambar - BKPM Acara 16)</label>
            ...
            <small style="color: #64748b; font-size: 11px;">Format: JPG, PNG, WEBP (Maksimal 2MB).</small>
        </div>
        ...
        <button type="submit" class="btn btn-gold" style="background: var(--admin-gold); color: #fff; border: none; padding: 12px 24px; border-radius: 6px; font-weight: 700; cursor: pointer;">
            <i class="fa-solid fa-save"></i> Simpan Produk Baru
        </button>
    </form>
</div>
@endsection
