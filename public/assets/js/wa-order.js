  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      if (varianSelect.selectedIndex >= 0) {
        const selectedOpt = varianSelect.options[varianSelect.selectedIndex];
        if (satuan === 'Slop') {
          pricePerUnit = parseFloat(selectedOpt.getAttribute('data-price-slop')) || 
                         (parseFloat(selectedOpt.getAttribute('data-price-bal') || 0) / 20) || 50000;
        } else {
          pricePerUnit = parseFloat(selectedOpt.getAttribute('data-price-bal')) || 
                         parseFloat(selectedOpt.getAttribute('data-price')) || 1000000;
        }
      }
      const totalHarga = pricePerUnit * parseInt(jumlah || 1);
